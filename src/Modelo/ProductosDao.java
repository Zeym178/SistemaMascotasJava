...
public class ProductosDao {
    ...
    public List ListarProductos(){
       ...
       String sql = "SELECT pr.id AS id_categoria, pr.nombre AS nombre_categoria, p.* FROM categoria pr INNER JOIN productos p ON pr.id = p.categoria ORDER BY p.id DESC";
       try {
           con = cn.getConnection();
           ps = con.prepareStatement(sql);
           rs = ps.executeQuery();
    ...
    public Productos BuscarId(int id){
        Productos pro = new Productos();
        String sql = "SELECT pr.id AS id_categoria, pr.nombre AS nombre_categoria, p.* FROM categoria pr INNER JOIN productos p ON p.categoria = pr.id WHERE p.id = ?";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            ps.setInt(1, id);
